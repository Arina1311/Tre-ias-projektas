...
void RasymasIRezultatuFailaSav(const string& failoPavadinimas, char Pasirinkimas, const vector<Studentas>& grupe) {
    ofstream output(failoPavadinimas); // Kuriame nauja faila
    ...
    try {
        ...
        output << fixed << setprecision(2);
        output << left << setw(15) << "Vardas" << setw(15) << "Pavarde" << setw(20) << (Pasirinkimas == 'V' ? "Galutinis(Vid.)" : "Galutinis(Med.)") <<setw(20)<<"Saugojimo adresas"<<endl;
        output << "---------------------" << "---------------------" << "---------------------" << "---------------------" << endl;

        for (const auto& studentas : grupe) {
            output << left << setw(15) << studentas.Vardas << setw(15) << studentas.Pavarde << setw(20) << studentas.Pazymys <<setw(20)<< &studentas <<endl;
        }
