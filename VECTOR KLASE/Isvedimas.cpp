...
void RasymasIRezultatuFaila(const string& failoPavadinimas, char Pasirinkimas, const vector<Studentas>& grupe) {
    ...
    try {
        ...
        for (const auto& studentas : grupe) {
            output << studentas;  // Naudoti išvesties operatorių
        }
...
void RasymasIEkrana (char Pasirinkimas, const vector<Studentas>& grupe) {
    try {
        ...
        cout << left << setw(15) << "Vardas" << std::setw(15) << "Pavarde"
                  << setw(20) << (Pasirinkimas == 'V' ? "Galutinis(Vid.)" : "Galutinis(Med.)") << endl;
        cout << "---------------------" << "---------------------" << "---------------------" << endl;

        for (const auto& studentas : grupe) {
            cout << studentas;  // Naudoti išvesties operatorių
        }
